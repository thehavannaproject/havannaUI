...
import cardIcon1 from "@images/Card Icons/Card icon-1.svg";
import cardIcon2 from "@images/Card Icons/Card icon-2.svg";
import cardIcon3 from "@images/Card Icons/Card icon-3.svg";
import cardIcon from "@images/Card Icons/Card icon.svg";

const Card = (bgColor, title, description, image) => {
  return (
    <div className={`py-6 px-8  smallLaptop:h-[492px] rounded-xl shadow-lg `}>
      <Animate.Fade bottom>
        <div className="text-dark font-bold text-sm-15 lg:text-base pt-3 lg:pt-7 items-center">
          <div className="mb-12">
            <Image alt="CardIcons" height={84} src={image} width={84} />
          </div>
          <p className=" text-sm lg:text-sm-15">{title}</p>
        </div>
        <p className={`text-default font-mulish font-medium text-xs lg:text-sm mt-6 leading-[1.6rem]}`}>{description}</p>
      </Animate.Fade>
    </div>
  );
};
    ...
    <section className="bg-HavannaGreen-light px-5 font-mulish pt-8 lg:pt-28 pb-28 lg:pb-32 relative">
      <div className="container smallLaptop:px-[120px] mx-auto">
        <div className="w-full lg:w-6/12">
          <Animate.Fade bottom>
            <h2 className={`leading-10 text-[36px] md:text-3xl lg:text-4xl xl:text-5xl font-bold text-primary bigLaptop:w-[538px] `}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-[26px] pt-12 xl:pt-20   ">
          {whySection.map((data, index) => (
            <div key={index}>
              <Card bgColor="white" description={data.description} image={data.image} title={data.title} />
            </div>
          ))}
        </div>
      </div>
