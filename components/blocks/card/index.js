...
const Card = (bgColor, title, description, tag) => {
  return (
    <div className={`${bgColor === "green" ? styles.cardGreen : styles.cardYellow} pt-8 pb-12 px-8 `}>
      <div className="text-dark font-bold text-sm-15 lg:text-base pt-4 lg:pt-7 relative">
        <div className="relative">
          <SlantBox bgColor={bgColor === "green" ? "bg-secondary" : "bg-primary"} height="h-10" width="w-10" />
      ...
      <p className={`text-default text-xs lg:text-sm pt-4 lg:pt-6 ${styles.cardDescription}`}>{description}</p>
    </div>
  );
};
